...
/* ===== Press & Hold ===== */
function startPress() {
  if (bottleRetired || isHolding) return; if (reverseRafId) { cancelAnimationFrame(reverseRafId); reverseRafId = null; }
  isHolding = true; holdStart = performance.now(); startBgNoiseLoop(); rafId = requestAnimationFrame(updateHoldProgress);
}
function endPress() {
  if (bottleRetired || !isHolding) return; isHolding = false; if (rafId) cancelAnimationFrame(rafId); rafId = null;
...
function toVideoWhileHolding() {
  ...
  requestAnimationFrame(() => {
    vid.style.opacity = '1'; vignette.style.opacity = '1';
...
function getHoverCandidates(lang) {
  ...
  const base = 'ic-pill/';           // <-- la tua nuova cartella
  const list = [];

  // Con suffisso lingua
  exts.forEach(ext => list.push({
    name: `${base}ic_name_${L}.${ext}`,
    info: `${base}ic_info_${L}.${ext}`
  }));

  // Fallback senza lingua
  exts.forEach(ext => list.push({
    name: `${base}ic_name.${ext}`,
    info: `${base}ic_info.${ext}`
  }));
